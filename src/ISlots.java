...
public class ISlots extends Tip implements NumberInfo {
	...
	public void layout(Layout var1) {
		...
		if (this.attrs.length > 0) {
			...
			var2 = RichText.render(String.format(chanceStr, chc, Long.valueOf(Math.round(100.0D * this.pmin)), chc,
					Long.valueOf(Math.round(100.0D * this.pmax))), 0, new Object[0]).img;
			...
			for (int var6 = 0; var6 < this.attrs.length; ++var6) {
				BufferedImage var7 = PUtils.convolvedown((this.attrs[var6].layer(Resource.imgc)).img,
						new Coord(var3, var3), CharWnd.iconfilter);
			...
			var2 = RichText.render(String.format(chanceStr, chc, Integer.valueOf((int) Math.round(100.0D * this.pmin))),
					0, new Object[0]).img;
		...
		if (this.left > 0) {
			...
			var1.cmp.add(
					Text.slotFnd
							.render(this.left > 1 ? String.format(gildStr, Integer.valueOf(this.left)) : gild2Str).img,
					new Coord(10, var1.cmp.sz.y));
		}
