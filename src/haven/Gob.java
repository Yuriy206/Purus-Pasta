...
public class Gob implements Sprite.Owner, Skeleton.ModOwner, Rendered {
	...
	public Collection<Overlay> ols = new LinkedList<Overlay>() {
		public boolean add(Overlay item) {
			...
			if (glob.oc.getgob(id) != null) {
				// FIXME: extend ols with a method for adding sprites without
				// triggering changed.
	...
	private static final Gob.Overlay animalradius =
			new Gob.Overlay(new BPRadSprite(100.0F, -10.0F, BPRadSprite.smatDanger));
	private static final Map<Gob, Gob.Overlay> playerhighlight = new HashMap<Gob, Gob.Overlay>();
	public Boolean knocked = null; // knocked will be null if pose update
									// request hasn't been received yet
	public Type type = null;

	public enum Type {
		OTHER(0), DFRAME(1), TREE(2), BUSH(3), BOULDER(4), PLAYER(5), SIEGE_MACHINE(6), MAMMOTH(7), BAT(8), OLDTRUNK(9),
		GARDENPOT(10), MUSSEL(11), LOC_RESOURCE(12), FU_YE_CURIO(13), PLANT(16), MULTISTAGE_PLANT(17), MOB(32),
		BEAR(34), LYNX(35), TROLL(38), WALRUS(39), WOODEN_SUPPORT(64), STONE_SUPPORT(65), METAL_SUPPORT(66), TROUGH(67),
		BEEHIVE(68), WAGON(600), WALL(602), DREAMCATCHER(603), HOUSE(604);
	 ...
	 * hopefully it can at least serve as a source of inspiration to redo
	 * attributes properly in the future. There have already long been arguments
	 * for remaking GAttribs as well.
	 */
		...
		public static class FactMaker implements Resource.PublishedCode.Instancer {
			public Factory make(Class<?> cl) throws InstantiationException, IllegalAccessException {
				...
				if (ResAttr.class.isAssignableFrom(cl)) {
					try {
						final java.lang.reflect.Constructor<? extends ResAttr> cons =
								cl.asSubclass(ResAttr.class).getConstructor(Gob.class, Message.class);
						return (new Factory() {
							public ResAttr mkattr(Gob gob, Message dat) {
	...
	public void determineType(String name) {
		if (name.startsWith("gfx/terobjs/trees") && !name.endsWith("log") && !name.endsWith("oldtrunk"))
			...
			type = Type.WAGON;
		else if (name.endsWith("/stonemansion") || name.endsWith("/logcabin") || name.endsWith("/greathall")
				|| name.endsWith("/stonestead") || name.endsWith("/timberhouse") || name.endsWith("stonetower"))
			type = Type.HOUSE;
			...
			type = Type.DREAMCATCHER;
				...
				|| name.startsWith("gfx/terobjs/arch/brick") && !name.equals("gfx/terobjs/arch/brickwallgate")
						&& !name.equals("gfx/terobjs/arch/brickwallbiggate")
				|| name.startsWith("gfx/terobjs/arch/pole") && !name.equals("gfx/terobjs/arch/polegate")
						&& !name.equals("gfx/terobjs/arch/polebiggate")) // Excludes
																			// gates
			type = Type.WALL;
	...
	public boolean setup(RenderList rl) {
		...
		if (d != null) {
			 ...
			 * { GobHitbox.BBox bbox = GobHitbox.getBBox(this, true); if (bbox
			 * != null) { rl.add(new Overlay(new GobHitbox(this, bbox.a, bbox.b,
			 * true)), null); } } else { d.setup(rl); }
			 */
	...
	private static final ClassResolver<Gob> ctxr =
			new ClassResolver<Gob>().add(Glob.class, g -> g.glob).add(Session.class, g -> g.glob.sess);
