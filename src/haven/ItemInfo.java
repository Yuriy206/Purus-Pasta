	...
	public static class FactMaker implements Resource.PublishedCode.Instancer {
		public InfoFactory make(Class<?> cl) throws InstantiationException, IllegalAccessException {
			...
			try {
				...
				if (ItemInfo.class.isAssignableFrom(mkm.getReturnType()) && ((mod & Modifier.STATIC) != 0)
		...
		public Contents(Owner owner, List<ItemInfo> sub) {
			...
			for (ItemInfo info : sub) {
				if (info instanceof ItemInfo.Name) {
					...
					if (name.str != null) {
						// determine whether we are dealing with seeds by
						// testing for
						// the absence of decimal separator (this will work
						// irregardless of current localization)
