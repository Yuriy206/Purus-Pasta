...
public class WItem extends Widget implements DTarget {
	...
	public final AttrCache<Tex> itemnum = new AttrCache<>(this::info, AttrCache.map1s(GItem.NumberInfo.class,
			ninf -> new TexI(GItem.NumberInfo.numrender(ninf.itemnum(), ninf.numcolor()))));

	public final AttrCache<Double> itemmeter =
			new AttrCache<>(this::info, AttrCache.map1(GItem.MeterInfo.class, minf -> {
	...
	public void tick(double dt) {
		/*
		 * XXX: This is ugly and there should be a better way to ensure the
		 * resizing happens as it should, but I can't think of one yet.
		 */
	...
	public boolean mousedown(Coord c, int btn) {
		if (btn == 1) {
			if (ui.modctrl && ui.modmeta)
				...
				// server side transfer all identical: pass third argument -1
				// (or 1 for single item)
