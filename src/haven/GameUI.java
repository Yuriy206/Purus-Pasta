...
public class GameUI extends ConsoleHost implements Console.Directory {
	...
	private Coord getMeterPos(int x, int y) {
		return new Coord(portrait.c.x + portrait.sz.x + 10 + x * (IMeter.fsz.x + 5),
				portrait.c.y + y * (IMeter.fsz.y + 2));
	}
	...
	public class MainMenu extends Widget {
		public MainMenu() {
			super(menubg.sz());
			add(new MenuButton("rbtn-inv", 9,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Inventory ($col[255,255,0]{Tab})")) {
				public void click() {
			...
			add(new MenuButton("rbtn-equ", 5,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Equipment ($col[255,255,0]{Ctrl+E})")) {
				public void click() {
			...
			add(new MenuButton("rbtn-chr", 20,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Character Sheet ($col[255,255,0]{Ctrl+T})")) {
				public void click() {
			...
			add(new MenuButton("rbtn-bud", 2,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Kith & Kin ($col[255,255,0]{Ctrl+B})")) {
				public void click() {
			...
			add(new MenuButton("rbtn-opt", 15,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Options ($col[255,255,0]{Ctrl+O})")) {
				public void click() {
			...
			add(new MenuButton("rbtn-dwn", 83,
					Resource.getLocString(Resource.BUNDLE_LABEL, "Menu Search ($col[255,255,0]{Shift+S})")) {
				public void click() {
	...
	public boolean globtype(char key, KeyEvent ev) {
		if (key == ':') {
			...
			return true;
		} else if (!Config.disabledrinkhotkey
				&& (ev.getKeyCode() == KeyEvent.VK_BACK_QUOTE || (Config.iswindows && Utils.getScancode(ev) == 41))) {
			maininv.drink(100);
			return true;
	...
	public class NKeyBelt extends Belt implements DTarget, DropTarget {
		...
		public NKeyBelt() {
			...
			adda(new IButton("gfx/hud/hb-btn-chat", "", "-d", "-h") {
				...
				{
					this.tooltip = RichText
							.render(Resource.getLocString(Resource.BUNDLE_LABEL, "Chat ($col[255,255,0]{Ctrl+C})"), 0);
