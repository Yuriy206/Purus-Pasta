		...
		while (!buf.eom()) {
			...
			case 3:
				...
				minfilter = new int[] { GL.GL_NEAREST, GL.GL_LINEAR, GL.GL_NEAREST_MIPMAP_NEAREST,
						GL.GL_NEAREST_MIPMAP_LINEAR, GL.GL_LINEAR_MIPMAP_NEAREST, GL.GL_LINEAR_MIPMAP_LINEAR, }[minf];
				break;
			case 4:
	...
	private class Real extends TexL {
		...
		private BufferedImage rd(final byte[] data) {
			return (AccessController.doPrivileged(new PrivilegedAction<BufferedImage>() {
				/*
				 * This can crash if not privileged due to ImageIO creating
				 * tempfiles without doing that privileged itself. It can very
				 * much be argued that this is a bug in ImageIO.
				 */
