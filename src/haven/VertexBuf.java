...
public class VertexBuf {
		...
		public FloatArray(int n, FloatBuffer data) {
			...
			if (data.capacity() % n != 0)
				throw (new RuntimeException(
						String.format("float-array length %d does not match element count %d", data.capacity(), n)));
			this.data = data;
		}
	...
	public abstract static class IntArray extends AttribArray {
		...
		public IntArray(int n, IntBuffer data) {
			...
			if (data.capacity() % n != 0)
				throw (new RuntimeException(
						String.format("int-array length %d does not match element count %d", data.capacity(), n)));
			this.data = data;
		}
	...
	public static class VertexArray extends FloatArray implements GLArray, MorphedMesh.MorphArray {
		...
		/*
		 * XXX: It feels very much like morphing should be layered strictly
		 * above VertexBuf, but I can't quite see an alternative to this at this
		 * point.
		 */
	...
	public static class NamedFloatArray extends FloatArray implements GLArray {
		...
		public Object progid(GOut g) {
			/*
			 * XXX: This is not a good ID, as it doesn't intern locations in
			 * various programs.
			 */
