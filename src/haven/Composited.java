...
public class Composited implements Rendered, MapView.Clickable {
	 ...
	 * this.id; if (r instanceof Model) { Model mod = (Model) r; if (mod.id >=
	 * 0) return (new CompositeClick(this, 0x01000000 | ((mod.id & 0xff) <<
	 * 8))); } else if (r instanceof Equ) { Equ equ = (Equ) r; if (equ.id >= 0)
	 * return (new CompositeClick(this, 0x02000000 | ((equ.id & 0xff) << 16)));
	 * } else if (r instanceof FastMesh.ResourceMesh) { FastMesh.ResourceMesh rm
	 * = (FastMesh.ResourceMesh) r; if ((id & 0xff000000) == 2) return (new
	 * CompositeClick(this, id & 0xffff0000 | (rm.id & 0xffff))); } return
	 * (this); } }
	 * 
