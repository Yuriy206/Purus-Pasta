	...
	public class Future<T> implements Runnable, Prioritized {
		...
		public void run() {
			...
			try {
				...
				running = null;
				/*
				 * XXX: This is a race; a cancelling thread could have gotten
				 * the thread reference via running and then interrupt this
				 * thread after interrupted() returns. There is no obvious
				 * elegant solution, though, and the risk should be quite low.
				 * Fix if possible.
				 */
				Thread.interrupted();
			}
