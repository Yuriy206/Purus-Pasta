...
public class Map {
	...
	private void initGeography() {
		...
		for (int x = -origintile; x < origintile; x++) {
			for (int y = -origintile; y < origintile; y++) {
				...
				// calculate waypoints located on the angular bisector of the
				// corner
				int wax = ca.x - 1;
				int way = ca.y - 1;
				int wbx = cb.x + 1;
				...
				// exclude tiles near map edges so we won't need to do bounds
	...
	public void addGob(Gob gob) {
		...
		// since non 90 degrees incremental rotation is wonky we slightly
		// increase the bounding box for such gobs
		...
		// FIXME: rotating rasters realiably to mach actual bounding boxes is
		// very tricky. need better way...
		...
		// exclude gobs near map edges so we won't need to do bounds checks all
		// over the place
	...
	public void excludeGob(Gob gob) {
		...
		// exclude the gob if it's near map edges so we won't need to do bounds
		// checks all later on
	...
	private void sanitizeWaypoints() {
		for (int i = 0; i < sz; i++) {
			...
			/*
			 * for (int i = 0; i < sz - 2*plbbox; ) { for (int j = 0; j < sz -
			 * 2*plbbox; ) { int cx = 0; int cy = 0; int cnt = 0; for (int x =
			 * 0; x < 2*plbbox; x++) { for (int y = 0; y < 2*plbbox; y++) { if
			 * (map[i+x][j+y] == CELL_WP) { cx += i + x; cy += j + y; cnt++; } }
			 * }
			 * 
			 * if (cnt == 0) { j++; continue; }
			 * 
			 ...
			 * }
			 * 
			 * for (int x = 0; x < 2*plbbox; x++) { for (int y = 0; y <
			 * 2*plbbox; y++) { if (map[i+x][j+y] == CELL_WP) { map[i+x][j+y] =
			 * CELL_FREE; } } } map[cx][cy] = CELL_WP; dbg.dot(cx, cy,
			 * Color.YELLOW);
			 * 
			 * j+= 2*plbbox; } i++; }
			 */
	...
	public Iterable<Edge> main() {
		...
		// clear area around starting position in case char is on the bounding
		// box boundary
