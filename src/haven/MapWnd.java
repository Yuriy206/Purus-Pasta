...
public class MapWnd extends Window {
	...
	public void tick(double dt) {
		...
		if (visible && (markerseq != view.file.markerseq)) {
			if (view.file.lock.readLock().tryLock()) {
				try {
					List<Marker> markers =
							view.file.markers.stream().filter(filter).collect(java.util.stream.Collectors.toList());
					markers.sort(mcmp);
	...
	private static final Pair[] filters =
			new Pair[] { new Pair<>("-- All --", null), new Pair<>("--- Custom ---", "flg"),
					new Pair<>("Abyssal Chasm", "abyssalchasm"), new Pair<>("Ancient Windthrow", "windthrow"),
					new Pair<>("Clay Pit", "claypit"), new Pair<>("Crystal Rock", "crystalpatch"),
					...
					new Pair<>("Jotun Mussel", "jotunmussel"), new Pair<>("Quest Givers", "qst"),
					new Pair<>("Salt Basin", "saltbasin"), new Pair<>("Swirling Vortex", "watervortex") };
