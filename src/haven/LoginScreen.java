...
public class LoginScreen extends Widget {
	...
	private void StartUpdaterThread() {
		Thread statusupdaterthread = new Thread(new Runnable() {
			public void run() {
				try {
					URL url = new URL("http://www.havenandhearth.com/mt/srv-mon"); // URL
																					// to
																					// connect
					HttpURLConnection con = (HttpURLConnection) url.openConnection(); // Initialise
																						// connection
					InputStream is;
					...
					while (true) {
						...
						} else { // Here we have one full line of
									// textif(stringBuf.contains("state hafen
									// ")) {
