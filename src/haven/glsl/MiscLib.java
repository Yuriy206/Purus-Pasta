	...
	public static final Uniform maploc = new Uniform.AutoApply(VEC3, PView.loc) {
		public void apply(GOut g, VarID loc) {
			...
			try {
				...
				/*
				 * XXX: WaterTile's obfog effect is the only thing that uses
				 * maploc, in order to get the precise water surface level.
				 * Arguably, maploc should be eliminated entirely and the obfog
				 * should pass the water level in a uniform instead. However,
				 * this works better for now, because with such a mechanic,
				 * Skeleton.FxTrack audio sprites would never complete if they
				 * get outside the map and stuck as constantly loading and never
				 * playing. Either way, when loading, the likely quite slight
				 * deviation between origin-Z and map-Z level probably doesn't
				 * matter a whole lot, but solve pl0x.
				 */
			}
	...
	public static final Function colblend = new Function.Def(VEC4) {
		{
			...
			code.add(new Return(vec4(add(mul(pick(base, "rgb"), sub(l(1.0), pick(blend, "a"))),
					mul(pick(blend, "rgb"), pick(blend, "a"))), pick(base, "a"))));
		}
	};
	public static final Function olblend = new Function.Def(VEC4) {
		{
			...
			Expression blend = param(IN, VEC4).ref();
			/*
			 * XXX: It seems weird indeed that Haven would be the only program
			 * having trouble with this, but on Intel GPUs, the sign() function
			 * very much appears to be buggy somehow (quite unclear just how; no
			 * explanation I can think of seems to make sense). Thus, this uses
			 * (x * 1000) instead of sign(x). Assuming color values mapped from
			 * uint8_t's, 1000 is enough to saturate the smallest deviations
			 * from 0.5.
			 */
			code.add(
					new Return(vec4(
							mix(mul(l(2.0), pick(base, "rgb"), pick(blend, "rgb")),
									sub(l(1.0),
											mul(l(2.0), sub(l(1.0), pick(base, "rgb")),
													sub(l(1.0), pick(blend, "rgb")))),
