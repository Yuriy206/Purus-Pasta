...
public class MorphedMesh extends FastMesh {
	private static Map<Morpher.Factory, Collection<MorphedBuf>> bufs =
			new CacheMap<Morpher.Factory, Collection<MorphedBuf>>(CacheMap.RefType.WEAK);
	...
	public static class MorphedBuf extends VertexBuf {
		...
		private static AttribArray[] ohBitterSweetJavaDays(VertexBuf from, Collection<Pair> pos, Collection<Pair> dir) {
			...
			for (int i = 0; i < from.bufs.length; i++) {
				MorphType type =
						(from.bufs[i] instanceof MorphArray) ? ((MorphArray) from.bufs[i]).morphtype() : MorphType.NONE;
