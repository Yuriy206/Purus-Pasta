...
public class OptWnd extends Window {
	...
	public final Panel main, video, audio, display, map, general, combat, control, uis, quality, flowermenus,
			soundalarms, hidesettings;
	...
	public class VideoPanel extends Panel {
		...
		public class CPanel extends Widget {
			...
			public CPanel(GLSettings gcf) {
				this.cf = gcf;
				final WidgetVerticalAppender appender =
						new WidgetVerticalAppender(withScrollport(this, new Coord(620, 350)));
				...
				CheckListbox disanimlist =
						new CheckListbox(320, Math.min(8, Config.disableanim.values().size()), 18 + Config.fontadd) {
							@Override
	...
	private void initDisplayFirstColumn() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(display, new Coord(620, 350)));
		appender.setVerticalMargin(VERTICAL_MARGIN);
	...
	private void initGeneral() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(general, new Coord(620, 350)));
	...
	private void initCombat() {
		...
		/*
		 * appender.add(new CheckBox("Show attack cooldown delta") { { a =
		 * Config.showcddelta; }
		 * 
		 * public void set(boolean val) { Utils.setprefb("showcddelta", val);
		 * Config.showcddelta = val; a = val; } });
		 */
	...
	private void initControl() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(control, new Coord(620, 350)));
	...
	private void initUis() {
		...
		appender.add(new CheckBox("Show quick hand slots") {
			...
			public void set(boolean val) {
				...
				} catch (ClassCastException e) { // in case we are at the login
													// screen
				}
			}
		});
	...
	private void initQuality() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(quality, new Coord(620, 350)));
		appender.setVerticalMargin(VERTICAL_MARGIN);
		appender.setHorizontalMargin(HORIZONTAL_MARGIN);
	...
	private void initFlowermenus() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(flowermenus, new Coord(620, 350)));
	...
	private void initHideMenu() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(hidesettings, new Coord(620, 350)));
		...
		appender.add(new HSlider(150, 0, 255, 0) {
			...
			public void changed() {
				...
				GobHitbox.fillclrstate =
						new States.ColState(new Color(Config.hidered, Config.hidegreen, Config.hideblue, 255));
			}
		});
	...
	private void initSoundAlarms() {
		final WidgetVerticalAppender appender =
				new WidgetVerticalAppender(withScrollport(soundalarms, new Coord(620, 350)));
			...
			if (en.hasMoreElements()) {
				...
				try (JarFile jar = urlcon.getJarFile()) {
					...
					while (entries.hasMoreElements()) {
						...
						// we assume that if tooltip localization exists then
						// the rest exist as well
	...
	private static final Pair[] combatkeys =
			new Pair[] { new Pair<>("[1-5] and [shift + 1-5]", 0), new Pair<>("[1-5] and [F1-F5]", 1) };
