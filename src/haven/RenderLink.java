				...
				l = new RenderLink() {
					...
					public Rendered make() {
						if (res == null) {
							...
							if (m == null)
								throw (new Sprite.ResourceException("Could not find specified mesh by ID " + meshid,
										getres()));
							if (M == null)
								throw (new Sprite.ResourceException("Could not find specified material by ID " + matid,
										getres()));
							res = M.apply(m);
						}
						return (res);
