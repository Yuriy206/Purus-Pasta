...
public class Fightsess extends Widget {
	...
	public Fightsess(int nact) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (i < 5)
				...
				keysftex[i] =
						Text.renderstroked(FightWnd.keysf[i - 5], Color.WHITE, Color.BLACK, Text.num12boldFnd).tex();
		}
	}
	...
	private static final Text.Furnace ipf = new PUtils.BlurFurn(
			new Text.Foundry(Text.serif, 18, new Color(128, 128, 255)).aa(true), 1, 1, new Color(48, 48, 96));
	...
	public void draw(GOut g) {
		...
		for (Buff buff : fv.buffs.children(Buff.class)) {
			Coord bc = Config.altfightui ? new Coord(gcx - buff.c.x - Buff.cframe.sz().x - 80, 180)
		...
		if (fv.current != null) {
			for (Buff buff : fv.current.buffs.children(Buff.class)) {
				Coord bc = Config.altfightui ? new Coord(gcx + buff.c.x + 80, 180)
						: pcc.add(buff.c.x + 20, buff.c.y + pho - Buff.cframe.sz().y);
				drawOpening(g, buff, bc);
			}
...

		{
			...
			if (now < fv.atkct) {
				...
				g.fellipse(cdc, Config.altfightui ? new Coord(24, 24) : new Coord(22, 22),
						Math.PI / 2 - (Math.PI * 2 * Math.min(1.0 - a, 1.0)), Math.PI / 2);
				g.chcolor();
				if (Config.showcooldown)
					g.atextstroked(Utils.fmt1DecPlace(cd), cdc, 0.5, 0.5, Color.WHITE, Color.BLACK, Text.num11Fnd);
			}
			g.image(cdframe, Config.altfightui ? new Coord(gameui().sz.x / 2, 200).sub(cdframe.sz().div(2))
					: cdc.sub(cdframe.sz().div(2)));
		}
		...
		if (fv.current != null) {
			try {
				...
				if (lastact != null) {
					Tex ut = lastact.get().layer(Resource.imgc).tex();
					Coord useul = Config.altfightui ? new Coord(gcx + 69 - ut.sz().x, 120)
							: pcc.add(usec2).sub(ut.sz().div(2));
					g.image(ut, useul);
					g.image(useframe, useul.sub(useframeo));
					double a = now - lastuse;
					if (a < 1) {
		...
		for (int i = 0; i < actions.length; i++) {
			Coord ca =
					Config.altfightui ? new Coord(gcx - 18, gui.sz.y - 250).add(actc(i)).add(16, 16) : pcc.add(actc(i));
	...
	public static final String[] keytips =
			{ "1", "2", "3", "4", "5", "Shift+1", "Shift+2", "Shift+3", "Shift+4", "Shift+5" };

	public Object tooltip(Coord c, Widget prev) {
		int cx = gameui().sz.x / 2;

		for (Buff buff : fv.buffs.children(Buff.class)) {
			Coord dc = Config.altfightui ? new Coord(cx - buff.c.x - Buff.cframe.sz().x - 80, 180)
					: pcc.add(-buff.c.x - Buff.cframe.sz().x - 20, buff.c.y + pho - Buff.cframe.sz().y);
		...
		if (fv.current != null) {
			for (Buff buff : fv.current.buffs.children(Buff.class)) {
				Coord dc = Config.altfightui ? new Coord(cx + buff.c.x + 80, 180)
						: pcc.add(buff.c.x + 20, buff.c.y + pho - Buff.cframe.sz().y);
				if (c.isect(dc, buff.sz)) {
					Object ret = buff.tooltip(c.sub(dc), prevtt);
					if (ret != null) {
		...
		for (int i = 0; i < actions.length; i++) {
			Coord ca = Config.altfightui ? new Coord(cx - 18, gameui().sz.y - 250).add(actc(i)).add(16, 16)
					: pcc.add(actc(i));
	...
	public boolean globtype(char key, KeyEvent ev) {
		...
		if (Config.combatkeys == 0) {
			if ((key == 0) && (ev.getModifiersEx()
					& (InputEvent.CTRL_DOWN_MASK | KeyEvent.META_DOWN_MASK | KeyEvent.ALT_DOWN_MASK)) == 0) {
