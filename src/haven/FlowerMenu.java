...
public class FlowerMenu extends Widget {
	...
	public class Petal extends Widget {
		...
		public Petal(String name) {
			super(Coord.z);
			this.name = name;
			text = ptf.render(Resource.getLocString(Resource.BUNDLE_FLOWER, name),
					name.startsWith("Travel ") ? Color.GREEN : Color.YELLOW);
			resize(text.sz().x + 25, ph);
		}

		public void move(double a, double r) {
			this.c = Coord.sc(a, r).sub(sz.div(2));
			// adjust horizontal position for potentially parallel petals to
			// avoid overlap
			if (r == rad) {
				for (Petal p : opts) {
