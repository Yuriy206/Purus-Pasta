...
public class HavenPanel extends GLCanvas implements Runnable, Console.Directory {
	...
	public static final GLState.Slot<GLState> proj2d =
			new GLState.Slot<GLState>(GLState.Slot.Type.SYS, GLState.class, global);
	...
	private static final Cursor emptycurs =
			Toolkit.getDefaultToolkit().createCustomCursor(TexI.mkbuf(new Coord(1, 1)), new java.awt.Point(), "");
	...
	private void initgl() {
		...
		addGLEventListener(new GLEventListener() {
			...
			public void display(GLAutoDrawable d) {
				...
				/*
				 * if((dump == null) || (dump.getDownstreamGL() != gl)) dump =
				 * new Debug.DumpGL((GL4bc)gl); if(Debug.kf2 && !Debug.pk2)
			...
			public void init(GLAutoDrawable d) {
				try {
					...
					if (h != null) {
						...
						// h.lsetprop("gl.version",
						// gl.glGetString(gl.GL_VERSION));
						// h.lsetprop("gl.renderer",
						// gl.glGetString(gl.GL_RENDERER));
						// h.lsetprop("gl.exts",
						// Arrays.asList(gl.glGetString(gl.GL_EXTENSIONS).split("
						// ")));
						// h.lsetprop("gl.caps",
						// d.getChosenGLCapabilities().toString());
					...
					gstate = new GLState() {
						public void apply(GOut g) {
							...
							if (g.gc.havefsaa()) {
								/*
								 * Apparently, having sample buffers in the
								 * config enables multisampling by default on
								 * some systems.
								 */
