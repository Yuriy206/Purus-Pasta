...
public class Skeleton {
	...
	private static float[] vqrot(float[] d, float[] v, float[] q) {
		 ...
		 * these.
		 */
		d[0] = (qw * qw * vx) + (2 * qw * qy * vz) - (2 * qw * qz * vy) + (qx * qx * vx) + (2 * qx * qy * vy)
				+ (2 * qx * qz * vz) - (qz * qz * vx) - (qy * qy * vx);
		d[1] = (2 * qx * qy * vx) + (qy * qy * vy) + (2 * qy * qz * vz) + (2 * qw * qz * vx) - (qz * qz * vy)
				+ (qw * qw * vy) - (2 * qw * qx * vz) - (qx * qx * vy);
		d[2] = (2 * qx * qz * vx) + (2 * qy * qz * vy) + (qz * qz * vz) - (2 * qw * qy * vx) - (qy * qy * vz)
				+ (2 * qw * qx * vy) - (qx * qx * vz) + (qw * qw * vz);
		return (d);
	}
	...
	public class Pose {
		...
		public Location bonetrans(final int bone) {
			return (new Location(Matrix4f.identity()) {
				...
				public Matrix4f fin(Matrix4f p) {
					if (cseq != seq) {
						Matrix4f xf = Transform.makexlate(new Matrix4f(),
								new Coord3f(gpos[bone][0], gpos[bone][1], gpos[bone][2]));
						if (grot[bone][0] < 0.999999) {
							float ang = (float) (Math.acos(grot[bone][0]) * 2.0);
							xf = xf.mul1(Transform.makerot(new Matrix4f(),
									new Coord3f(grot[bone][1], grot[bone][2], grot[bone][3]).norm(), ang));
						}
						update(xf);
						cseq = seq;
					}
				...
				public Matrix4f fin(Matrix4f p) {
					if (cseq != seq) {
						...
						if (rot[0] < 0.999999) {
							...
							xf = xf.mul1(
									Transform.makerot(new Matrix4f(), new Coord3f(rot[1], rot[2], rot[3]).norm(), ang));
						}
						update(xf);
						cseq = seq;
					}
		...
		public class BoneAlign extends Location {
			...
			public Matrix4f fin(Matrix4f p) {
				if (cseq != seq) {
					Coord3f cur = new Coord3f(gpos[tgt][0] - gpos[orig][0], gpos[tgt][1] - gpos[orig][1],
							gpos[tgt][2] - gpos[orig][2]).norm();
			...
			public PoseMod create(Skeleton skel, ModOwner owner, Resource res, Message sdt) {
				...
				for (ResPose p : res.layers(ResPose.class)) {
					if ((p.id < 0) || ((mask & (1 << p.id)) != 0)) {
						...
						if (itm != null && itm.selected && res.name.endsWith("/idle")
			...
			for (Track t : tracks) {
				...
				if (b == null)
					throw (new RuntimeException(
							"Bone \"" + t.bone + "\" in animation reference does not exist in skeleton " + skel));
				remap[b.idx] = t;
			}
			return (remap);
		}
