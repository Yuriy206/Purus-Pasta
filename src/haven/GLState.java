...
public abstract class GLState {
	...
	public static class Buffer {
		...
		public void copye(Buffer dest, Slot.Type type) {
			...
			for (int i = states.length; i < dest.states.length; i++) {
				if (i < idlist.length && idlist[i].type != type) // NOTE: hotfix
																	// for IOOB
																	// exception
					dest.states[i] = null;
			}
	...
	public static class Applier {
		...
		/*
		 * XXX: Arguably, there should be a way to more fundamentally affect the
		 * gl_Position generation code instead of this.
		 */
