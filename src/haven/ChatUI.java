...
public class ChatUI extends Widget {
	...
	public static final Color[] urgcols =
			new Color[] { null, new Color(0, 128, 255), new Color(255, 128, 0), new Color(255, 0, 0), };
		...
		public class NamedMessage extends Message {
			...
			public Text text() {
				...
				if ((r == null) || !nm.equals(cn)) {
					...
					r = fnd.render(RichText.Parser.quote(Config.chattimestamp ? timestamp(tf) : tf), w,
		...
		if (dm != null) {
			if (resizehoriz) {
				...
				if (gui.questpanel != null)
					gui.questpanel.c =
							new Coord(10, gui.sz.y - gui.chat.sz.y - gui.beltwdg.sz.y - gui.questpanel.sz.y - 10);
			}
		} else {
			super.mousemove(c);
		}
	...
	private static void save(String chatName, String text, String name) {
		if (Config.chatsave)
			save(chatName, Config.chattimestamp ? text.substring(0, 7) + " " + name + ":" + text.substring(7)
					: name + ": " + text);
	}
