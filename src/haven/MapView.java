...
public class MapView extends PView implements DTarget, Console.Directory, PFListener {
	...
	private static final Gob.Overlay rovltrough =
			new Gob.Overlay(new BPRadSprite(200.0F, -10.0F, BPRadSprite.smatTrough));
	private static final Gob.Overlay rovlbeehive =
			new Gob.Overlay(new BPRadSprite(151.0F, -10.0F, BPRadSprite.smatBeehive));
	...
	public abstract class Camera extends GLState.Abstract {
		...
		public void resized() {
			...
			proj.update(
					Projection.makefrustum(new Matrix4f(), -field, field, -aspect * field, aspect * field, 1, 5000));
		}
	...
	public class FollowCam extends Camera {
		...
		private float dist(float elev) {
			...
			return ((float) (((cd - (h / Math.tan(elev))) * Math.sin(elev - da) / Math.sin(da))
					- (h / Math.sin(elev))));
		}
		...
		public void tick(double dt) {
			...
			if (dist > 250) {
				...
				Coord3f cambase = new Coord3f(curc.x + ((float) Math.cos(tangl) * pd),
						curc.y + ((float) Math.sin(tangl) * pd), 0.0f);
			...
			for (o.y = -view; o.y <= view; o.y++) {
				for (o.x = -view; o.x <= view; o.x++) {
					...
					try {
						...
						// there seems to be a rare problem with fetching
						// gridcuts when teleporting, not sure why...
						// we ignore Defer.DeferredException to prevent the
						// client for crashing
					}
				}
			}
				...
				for (o.x = -view; o.x <= view; o.x++) {
					...
					for (int i = 0; i < visol.length; i++) {
						...
						if (visol[i] > 0) {
							...
							if (olcut != null)
								rl.add(olcut, GLState
										.compose(Location.xlate(new Coord3f((float) pc.x, -(float) pc.y, 0)), mats[i]));
						}
					}
				}
			}
	...
	private class Gobs implements Rendered {
		...
		public String toString() {
			return (String.format("%,dd %,dn %,dS %,ds %,do", dynamic.size(), newfags.size(), semistat.size(),
					semifags.size(), oldfags.size()));
		}
	}
	...
	public void setup(RenderList rl) {
		synchronized (glob) {
			if (glob.lightamb != null) {
				...
				DirLight light = new DirLight(lightamb, lightdif, lightspc,
	...
	private static final Text.Furnace polownertf =
			new PUtils.BlurFurn(new Text.Foundry(Text.serif, 30).aa(true), 3, 1, Color.BLACK);
	...
	public void draw(GOut g) {
		...
		try {
			...
			try {
				...
				// there seems to be a rare problem with fetching gridcuts when
				// teleporting, not sure why...
				// we ignore Defer.DeferredException to prevent the client for
				// crashing
			}
	...
	private class Click extends Hittest {
		...
		protected void hit(Coord pc, Coord2d mc, ClickInfo inf) {
			...
			if (clickb == 1)
				pllastcc = mc;
			// reset alt so we could walk with alt+lmb while having item on the
			// cursor
			...
			if (Config.proximityaggro && clickb == 1 && curs != null && curs.name.equals("gfx/hud/curs/atk")) {
				...
				if (target != null) {
					wdgmsg("click", target.sc, target.rc.floor(posres), 1, 0, 0, (int) target.id,
			...
			if (inf == null) {
				...
				if (ui.modmeta && ui.modctrl && clickb == 1 && gob != null) {
					...
					if (gameui().vhand == null) { // do not highlight when
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (button == 2) {
			...
			if (placing.lastmc != null)
				wdgmsg("place", placing.rc.floor(posres), (int) Math.round(placing.a * 32768 / Math.PI), button,
						ui.modflags());
			...
			public void hit(Coord pc, Coord2d mc, ClickInfo inf) {
				if ((Config.nodropping || Config.nodropping_all) && !ui.modctrl) {
					...
					if (Config.nodropping_all) {
						...
						// we came here because Config.nodropping is set, check
						// water tiles
	...
	public void aggroclosest() {
		...
		synchronized (oc) {
			...
			if (gobcls != null) {
				gameui().act("aggro");
				wdgmsg("click", gobcls.sc, Coord.z, 1, ui.modflags(), 0, (int) gobcls.id, gobcls.rc.floor(posres), 0,
						0);
				Gob pl = player();
