...
public class StatusWdg extends Widget {
	...
	private static final Tex hearthlingsplayingdef = Text
			.render(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Players: %s"), "?"), Color.WHITE).tex();
	private static final Tex pingtimedef = Text
			.render(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Ping: %s ms"), "?"), Color.WHITE).tex();
	...
	// GNU ping IPv4: 64 bytes from ansgar.seatribe.se (213.239.201.139):
	// icmp_seq=1 ttl=50 time=72.5 ms
	// GNU ping IPv6: 64 bytes from ansgar.seatribe.se: icmp_seq=1 ttl=53
	// time=15.3 ms
	private final static Pattern pattern =
			Pattern.compile(Config.iswindows ? ".+?=(\\d+)[^ \\d\\s]" : ".+?time=(\\d+\\.?\\d*) ms");
	...
	private void updatepingtime() {
		...
		synchronized (this) {
			pingtime = Text.render(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Ping: %s ms"), ping),
					Color.WHITE).tex();
		}
	}
				...
				try {
					...
					while ((line = br.readLine()) != null) {
						if (line.startsWith("users ")) {
							...
							players = Text.render(
									String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Players: %s"), p),
									Color.WHITE).tex();
						}

						// Update ping at least every 5 seconds.
						// This of course might take more than 5 seconds in case
						// there were no new logins/logouts
					...
					// don't print socket exceptions when network is unreachable
					// to prevent console spamming on bad connections
