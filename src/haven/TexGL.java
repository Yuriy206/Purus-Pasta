...
public abstract class TexGL extends Tex {
	...
	public static class TexClip extends GLState {
		public static final Slot<TexClip> slot =
				new Slot<TexClip>(Slot.Type.GEOM, TexClip.class, HavenPanel.global, TexDraw.slot);
	 ...
	 * if(invert) { for(int y = 0; y < tdim.y / 2; y++) { int to = y * tdim.x *
	 * 4, bo = (tdim.y - y - 1) * tdim.x * 4; for(int o = 0; o < tdim.x * 4;
	 * o++, to++, bo++) { byte t = buf[to]; buf[to] = buf[bo]; buf[bo] = t; } }
	 * } WritableRaster raster = Raster.createInterleavedRaster(new
	 * DataBufferByte(buf, buf.length), tdim.x, tdim.y, 4 * tdim.x, 4, new int[]
	 * {0, 1, 2, 3}, null); return(new BufferedImage(TexI.glcm, raster, false,
	 * null)); }
	 * 
