...
public class HashDirCache implements ResCache {
	...
	/*
	 * These locks should never have to be waited for long at all, so blocking
	 * interruptions until complete should be perfectly okay.
	 */
	...
	private File lookup(String name, boolean creat) throws IOException {
		...
		synchronized (monitors) {
			/*
			 * Apparently, Java doesn't allow two threads in one JVM to lock the
			 * same file...
			 */
	...
	private Iterator<String> list(boolean filter) {
		final File[] files = base.listFiles(f -> (f.getName().length() >= 18) && (f.getName().charAt(16) == '.')
