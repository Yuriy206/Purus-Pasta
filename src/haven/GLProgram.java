				...
				public void run(GL2 rgl) {
					...
					if (buf[0] != 1) {
						...
						if (buf[0] > 0) {
							...
							/*
							 * The "platform's default charset" is probably a
							 * reasonable choice.
							 */
			...
			r = new VarID(name) {
				public void run(GL2 gl) {
					if ((this.id = glob.uniform(gl, name)) < 0)
						throw (new UnknownExternException("Unknown uniform name: " + name, GLProgram.this, "uniform",
								name));
				}
			};
			g.gl.bglSubmit(r);
			...
			r = new VarID(name) {
				public void run(GL2 gl) {
					if ((this.id = glob.attrib(gl, name)) < 0)
						throw (new UnknownExternException("Unknown attribute name: " + name, GLProgram.this, "attrib",
								name));
				}
			};
			g.gl.bglSubmit(r);
