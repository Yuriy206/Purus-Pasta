...
public class MenuGrid extends Widget {
	...
	public final static RichText.Foundry ttfnd = new RichText.Foundry(TextAttribute.FAMILY, Text.cfg.font.get("sans"),
			TextAttribute.SIZE, Text.cfg.tooltipCap); // aa(true)
	...
	public static class Pagina implements java.io.Serializable, ItemInfo.Owner {
		...
		private static final OwnerContext.ClassResolver<Pagina> ctxr = new OwnerContext.ClassResolver<Pagina>()
				.add(Glob.class, p -> p.glob).add(Session.class, p -> p.glob.sess);
	...
	private Tex glowmask(Pagina pag) {
		...
		if (ret == null) {
			ret = new TexI(PUtils.glowmask(PUtils.glowmask(pag.res().layer(Resource.imgc).img.getRaster()), 4,
					new Color(32, 255, 32)));
		...
		if (sub.size() > 0) {
			...
			if (ad[0].equals("@")) {
				...
				if (Config.confirmmagic && r.res.get().name.startsWith("paginae/seid/")) {
					...
					confirmwnd.add(
							new Label(Resource.getLocString(Resource.BUNDLE_LABEL,
									"Using magic costs experience points. Are you sure you want to proceed?")),
							new Coord(10, 20));
					confirmwnd.pack();
