...
public class AudioSprite {
	...
	public static class ClipSprite extends Sprite {
		...
		public boolean tick(int dt) {
			/*
			 * XXX: This is slightly bad, because virtual sprites that are stuck
			 * as loading (by getting outside the map, for instance), never play
			 * and therefore never get done, effectively leaking. For now, this
			 * is seldom a problem because in practive most (all?) virtual
			 * audio-sprites come from Skeleton.FxTrack which memoizes its
			 * origin instead of asking the map for it, but also see comment in
	...
	public static class RepeatSprite extends Sprite implements Gob.Overlay.CDel {
		...
		public RepeatSprite(Owner owner, Resource res, final Resource.Audio beg, final Resource.Audio clip,
				Resource.Audio end) {
			super(owner, res);
			this.end = end;
