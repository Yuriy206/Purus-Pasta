...
public class Window extends Widget implements DTarget {
	...
	public static final Text.Furnace cf =
			new Text.Imager(new PUtils.TexFurn(new Text.Foundry(Text.sans, Text.cfg.wndCap).aa(true), ctex)) {
				protected BufferedImage proc(Text text) {
					return (rasterimg(blurmask2(text.img.getRaster(), 1, 1, Color.BLACK)));
				}
			};
	public static final IBox wbox =
			new IBox("gfx/hud/wnd", "tl", "tr", "bl", "br", "extvl", "extvr", "extht", "exthb") {
	...
	public void draw(GOut g) {
		...
		drawframe(g);
		/*
		 * wbox.draw(g, wtl, wsz); if(cap != null) { int w = cap.sz().x; int y =
		 * wtl.y - capo; g.image(cl, new Coord(wtl.x + (wsz.x / 2) - (w / 2) -
		 * cl.sz().x, y)); g.image(cm, new Coord(wtl.x + (wsz.x / 2) - (w / 2),
		 * y), new Coord(w, cm.sz().y)); g.image(cr, new Coord(wtl.x + (wsz.x /
		 * 2) + (w / 2), y)); g.image(cap.tex(), new Coord(wtl.x + (wsz.x / 2) -
		 * (w / 2), y + capio)); }
		 */
		super.draw(g);
	}
