...
public class RealmBuff extends Buff implements ResOwner {
	...
	public static final ClassResolver<UI> uictx =
			(new ClassResolver<UI>()).add(Glob.class, (var0) -> var0.sess.glob).add(Session.class, (var0) -> var0.sess);
	...
	public Object tooltip(Coord var1, Widget var2) {
		try {
			if (this.rtip == null) {
				...
				if (var3.size() < 2) {
					var4 = ItemInfo.catimgs(0, new BufferedImage[] { var4, Text.render(
							Resource.getLocString(Resource.BUNDLE_LABEL, "This realm confers no bonus.")).img });
				}

				Pagina var5 = this.res.get().layer(Resource.pagina);
				if (var5 != null) {
					var4 = ItemInfo.catimgs(0,
							new BufferedImage[] { var4,
									RichText.render("\n" + Resource.getLocString(Resource.BUNDLE_LABEL, var5.text), 200,
											new Object[0]).img });
				}
