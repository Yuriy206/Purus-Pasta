	...
	private void updpos(GOut g) {
		if (bollar.size() < 1) {
			...
			while (it.hasNext()) {
				...
				var7.put(r).put(this.g).put(b).put(a
						* (float) Utils.clip(1.0D - Math.pow((double) (var9.t / life), (double) fadepow), 0.0D, 1.0D));
			}
	...
	final static class SM1 implements ShaderMacro {
		...
		final static class ISmoke$1$1 implements Macro1<Expression> {
			...
			public Expression expand(Expression var1) {
				...
				var10004[0] = VertexContext
						.projxf(Cons.add(new Expression[] { this.var2.vctx.eyev.depref(), Cons.vec4(new Expression[] {
								bollsz.ref() /* ISmoke.access$000 */, Cons.l(0.0D), Cons.l(0.0D), Cons.l(0.0D) }) }));
				var10000[0] = Cons.sub(var10003.call(var10004),
						this.pdiv.call(new Expression[] { this.var2.vctx.posv.depref() }));
