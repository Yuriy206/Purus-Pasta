...
public class Ingredient extends Tip {
	...
	public String descr() {
		return this.val == null ? Resource.getLocString(Resource.BUNDLE_LABEL, name)
				: String.format("%s (%d%%)", Resource.getLocString(Resource.BUNDLE_INGREDIENT, name),
						Integer.valueOf((int) Math.floor(this.val.doubleValue() * 100.0D)));
	}
	...
	public static class Line extends Tip {
		...
		public BufferedImage tipimg() {
			...
			switch (all.size()) {
			case 1:
				ings.append(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Made from %s"),
						all.get(0).descr()));
				break;
			case 2:
				ings.append(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Made from %s and %s"),
						all.get(0).descr(), all.get(1).descr()));
				break;
			case 3:
				ings.append(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Made from %s, %s and %s"),
						all.get(0).descr(), all.get(1).descr(), all.get(2).descr()));
				break;
			case 4:
				ings.append(String.format(Resource.getLocString(Resource.BUNDLE_LABEL, "Made from %s, %s, %s and %s"),
						all.get(0).descr(), all.get(1).descr(), all.get(2).descr(), all.get(3).descr()));
				break;
			}
